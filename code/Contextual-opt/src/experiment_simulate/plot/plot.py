...
if __name__ == '__main__':
    ...
    for i in range(2):
        # log = pd.read_csv(os.path.dirname(__file__)+"/plot_log_{}_4dim.csv".format(sp[i]))
        log = pd.read_csv(os.path.dirname(__file__)+"/plot_log_push_{}dim.csv".format(sp[i]))
        ...
        if i == 0:
            ax[i].legend((p3[0],p6[0],p4[0],p5[0],p7[0]), ("Proposed","Proposed (model output)","CMA-ES","Contextual CMA-ES","WS-CMA-ES"),ncol=5,loc='upper center', bbox_to_anchor=(1.03, -.2))
            # ax[i].legend((p3[0],p4[0],p5[0],p7[0]), ("Proposed","CMA-ES","Contextual CMA-ES","WS-CMA-ES"),ncol=5,loc='upper center', bbox_to_anchor=(1.03, -.2))
