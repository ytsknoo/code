...
def get_base_fetch_env(RobotEnvClass: Union[MujocoPyRobotEnv, MujocoRobotEnv]):
    ...
    class BaseFetchEnv(RobotEnvClass):
        ...
        #追加
        def _set_goal_pos(self,goal_pos):
            _goal_pos = [goal_pos[0],goal_pos[1],0]
            if self.has_object:
                goal = self.initial_gripper_xpos[:3] + _goal_pos
                goal += self.target_offset
                goal[2] = self.height_offset
            else:
                goal = self.initial_gripper_xpos[:3] + _goal_pos
...
class MujocoPyFetchEnv(get_base_fetch_env(MujocoPyRobotEnv)):
    ...
    def _reset_sim(self):
        ...
        if self.has_object:
            ...
            self.sim.data.set_joint_qpos("object0:joint", object_qpos) #絶対ここ
...
class MujocoFetchEnv(get_base_fetch_env(MujocoRobotEnv)):
    ...
    def _reset_sim(self):
        ...
        # Randomize start position of object.多分物体の設定
        if self.has_object:
            object_xpos = self.initial_gripper_xpos[:2]
            while np.linalg.norm(object_xpos - self.initial_gripper_xpos[:2]) < 0.1:
                object_xpos = self.initial_gripper_xpos[:2] + self.np_random.uniform(
    ...
    def set_obj_pos(self,obj_pos):
        ...
        if self.has_object:
            #アームを基準に
            # object_xpos = self.initial_gripper_xpos[:2]
            object_xpos = self.initial_gripper_xpos[:2] + obj_pos
            object_qpos = self._utils.get_joint_qpos(
                self.model, self.data, "object0:joint"
            )
            assert object_qpos.shape == (7,)
